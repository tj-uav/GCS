...
class Obstacle():
    ...
    def inMe(self, x, y, z):
        return (y - self.y) ** 2 + (x - self.x) ** 2 < ((self.r + radius_tolerance) ** 2) and (self.z + height_tolerance) >= z
    ...
    def nbrs(self, obs):
        ...
        for dp in np.arange(-pi / 12, pi / 12 + dPhi, dPhi):
            ...
            for dt in np.arange(-pi / 6, pi / 6 + dTheta, dTheta):
                lst.append(Node(self.x + rho * cos(self.theta + dt) * sin(self.phi + dp),
...
def read_mission():
    ...
    for obstacle in mission_dict["stationaryObstacles"]:
        ...
        ax.add_artist(Obstacle(dx, dy, height, rad).plottable())
...
def display():
    ...
    for waypoint in waypoints:
        waypoint_path_x.append(waypoint[0])
        waypoint_path_y.append(waypoint[1])
        waypoint_path_z.append(waypoint[2])
...
def main():
    ...
    display()
    print("waypoints:")
    print(waypoint_path_x)
    print(waypoint_path_y)
